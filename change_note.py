            ...
            with open("notes_file.txt", "r", encoding="utf-8") as file:
                ...
                if lines_qty == 0:
                    ...
                    if indx == None:
                        ...
                        note_title = input("Введите новый заголовок заметки и нажмите клавишу Enter: ")
                        note_text = input("Введите новый текст заметки и нажмите клавишу Enter: ")
                        storage_date_time = str(datetime.datetime.now().strftime("%d.%m.%Y %H:%M:%S"))
                        note_ID = ID_to_change

                        fl = get_file_lines()
                        fl[int(indx)] = f"{note_ID};{note_title};{note_text};{storage_date_time}\n"
